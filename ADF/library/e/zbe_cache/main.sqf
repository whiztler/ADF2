...
call compileFinal preprocessFileLineNumbers "ADF\library\e\zbe_cache\zbe_functions.sqf";
	...
	while {true} do {
		...
		zbe_players = allPlayers - entities "HeadlessClient_F";
		{
			...
			if !(_disable && (_x in zbe_cachedGroups)) then {
				...
				[zbe_aiCacheDist, _x, zbe_minFrameRate, zbe_debug] execFSM "ADF\library\e\zbe_cache\zbe_aiCaching.fsm";
			};
		} forEach allGroups;
	};
};
	...
	while {true} do {
		_assetscar = zbe_centerPOS nearEntities ["LandVehicle", zbe_mapside];
		{
			if !(_x in zbe_cached_cars) then {
				...
				[_x, zbe_vehicleCacheDistCar] execFSM "ADF\library\e\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetscar;
		...
		_assetsair = zbe_centerPOS nearEntities ["Air", zbe_mapside];
		{
			if !(_x in zbe_cached_air) then {
				...
				[_x, zbe_vehicleCacheDistAir] execFSM "ADF\library\e\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsair;
		...
		_assetsboat = zbe_centerPOS nearEntities ["Ship", zbe_mapside];
		{
			if !(_x in zbe_cached_boat) then {
				...
				[_x, zbe_vehicleCacheDistBoat] execFSM "ADF\library\e\zbe_cache\zbe_vehicleCaching.fsm";
			};
		} forEach _assetsboat;
...
// if (!isDedicated) then {execFSM "ADF\library\e\zbe_cache\zbe_clientObjectDrawAuto.fsm";};